...
class ASRAPI:
    ...
    def batch_process(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            # 提交所有任务
            future_to_input = {
                executor.submit(self._process_single_with_output, input_path, output_format, output_dir): input_path
            ...
            for future in as_completed(future_to_input):
                ...
                try:
                    ...
                    self.logger.error(f"处理异常 {input_path}: {str(e)}")
                    results['failed'].append(input_path)
        
        self.logger.info(f"批量处理完成 - 成功: {len(results['success'])}, 失败: {len(results['failed'])}")
        return results
    ...
    def _process_single_with_output(
        ...
        try:
            ...
            return self.process_file(input_path, output_format, output_path)
...
def batch_process(input_paths: List[str], output_format: str = 'srt', **kwargs) -> dict:
    """
    简化版批量处理函数
    ...
    """
    api = ASRAPI(**kwargs)
    return api.batch_process(input_paths, output_format)


def process_directory(input_dir: str, output_format: str = 'srt', **kwargs) -> dict:
    """
    简化版目录处理函数
    ...
    """
    api = ASRAPI(**kwargs)
    return api.process_directory(input_dir, output_format, **kwargs)
